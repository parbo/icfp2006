...
DXX      i = i + I
DXXV     IF i > pwdcount THEN GOTO DXXXI
DXXX     GOTO DXI
DXXXI    i = I
DXXXV    k = k + I
DXL      IF k > LVII THEN GOTO DL
DXLV     GOTO DXI
DL       i = I
DLI      j = j + I
DLII     k = XLVIII
DLV      IF j > LVII THEN GOTO DLXXX
DLX      GOTO DXI
DLXV     PRINT "found match!! for user " + username + CHR(X)
DLXX     PRINT "password: " + words(i) + CHR(j) + CHR(k)
DLXXV    END
DLXXX    PRINT "no matches for user " + username + CHR(X)
DLXL     END
